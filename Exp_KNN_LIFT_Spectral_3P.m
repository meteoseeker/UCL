...
dataset_names = {'lost'};% input datasets: synthetic datasets ('glass',...) or real wolrd datasets ('lost',...) 
...
num_FL = 1; % (r) number of false postive labels, can be set as [1, 2, 3]. num_FL is valid when mode is 'random'
epsilon = 0.1; % probability for picking co-occurring label as false postive label, can be set as [0.1, 0.2, ..., 0.7]. epsilon is valid when mode is 'co-occur'

% setting dir for saving log and learned model
exp_dir = 'Exps_UCL_KNN';
...
%% Evaluating KNN_LIFT_Spectral_3P algorithm
% setting paramters for algorithm
algorithm_name = 'KNN_LIFT_Spectral_3P';
...
for dataset_name = dataset_names
    ...
    for i = 1:nfold
        ...
        rng(5); % random seed
        tic
        [accuracy,predictLabel,outputValue] = PL_KNN_LS(new_train_data, train_partial_y', new_val_data, val_y');
        test_times(i) = toc;
